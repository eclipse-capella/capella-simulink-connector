...
addLabel(file, 'Capella Layer', 'SA');
if isfile('data/SA/SA_container.sldd')
	SA_container = Simulink.data.dictionary.open('data/SA/SA_container.sldd');
else
	SA_container = Simulink.data.dictionary.create('data/SA/SA_container.sldd');
end
addDataSource(SA_container, 'SA_FunctionalExchanges_Categories.sldd');
saveChanges(SA_container);
clear SA_container;
file = proj.addFile('data/SA/SA_container.sldd');
addLabel(file, 'Capella Layer', 'SA');
...
addLabel(file, 'Capella Layer', 'SA');
new_system('SA_container');
set_param('SA_container', 'Tag', 'ModelerId:8260089b-12f0-459c-8b5e-bb1f95b1b3dd');
set_param('SA_container', 'DataDictionary', 'SA_container.sldd');
add_block('CapellaElements/ModelRef/Model_Function', 'SA_container/Reference_to_leaf2');
set_param('SA_container/Reference_to_leaf2', 'Tag', 'ModelerId:48d0665e-6da4-4fea-9a2c-b8352e052536');
set_param('SA_container/Reference_to_leaf2', 'ModelFile', 'SA_leaf2')
add_block('CapellaElements/ModelRef/Model_Function', 'SA_container/Reference_to_leaf1');
set_param('SA_container/Reference_to_leaf1', 'Tag', 'ModelerId:04bdd067-add9-4cac-a7fa-f96c7f970dd7');
set_param('SA_container/Reference_to_leaf1', 'ModelFile', 'SA_leaf1')
handles = get_param('SA_container/Reference_to_leaf1', 'porthandles');
set_param(handles.Outport(1), 'Tag', 'ModelerId:33e462c5-a19d-4381-a62f-19968e28a819');
clear handles;
handles = get_param('SA_container/Reference_to_leaf1', 'PortHandles');
source = handles.Outport(1);
handles = get_param('SA_container/Reference_to_leaf2', 'PortHandles');
target = handles.Inport(1);
line = add_line('SA_container', source, target, 'autorouting','smart');
...
try
	Simulink.BlockDiagram.arrangeSystem('SA_container');
catch
end
save_system('SA_container', fullfile(project_path, 'models/SA/SA_container.slx'));
file = proj.addFile('models/SA/SA_container.slx');
addLabel(file, 'Capella Layer', 'SA');
