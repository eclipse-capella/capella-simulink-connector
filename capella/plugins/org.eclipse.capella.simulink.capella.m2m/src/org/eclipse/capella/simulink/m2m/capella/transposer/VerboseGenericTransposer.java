...
import org.eclipse.capella.simulink.m2m.capella.rule.common.AbstractCapellaToPivotRule;
  ...
  /**
   * Copied from org.polarsys.kitalpha.transposer.generic.GenericTransposer
   * Removed catch clauses so that end user can get the errors
   */
public class VerboseGenericTransposer implements ITransposer, ITransposerWorkflow {
   ...
   * 
   */
  public VerboseGenericTransposer(String purpose_p, String mappingId_p) {
 ...
 * @see org.polarsys.kitalpha.transposer.api.ITransposer#transpose(java.util.Collection)
   */
public void verboseTranspose(Collection<Object> selection_p, TransposerConfiguration configuration_p, Comparator<Vertex<?>> comparator_p,IProgressMonitor monitor_p) throws Exception {
	 ...
	 * @throws Exception 
	 */
	private void callPostExecutionActivities(TransposerConfiguration configuration_p, IProgressMonitor monitor_p) throws Exception {
		if (configuration_p == null) {
			return;
		}

		complementActivitiesParameters(configuration_p.getPostExecutionActivities());
		CadenceLauncher.cadence(TRANSPOSER_WORKFLOW, POST_EXECUTION, configuration_p.getPostExecutionActivities(), monitor_p);
	}
		...
		return new GenericRulesHandler(purpose_p, mappingId_p) {
			  ...
			  public List<IPremise> getPremises(Object object_p) throws ComputePremisesException {
			      ...
			      if (applicablePossibility != null) {
			        ...
			        if (completeRule instanceof AbstractCapellaToPivotRule) 
			        	return ((AbstractCapellaToPivotRule)completeRule).getPremises((EObject) object_p, _context);
